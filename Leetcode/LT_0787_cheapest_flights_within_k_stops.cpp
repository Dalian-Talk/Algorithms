...
/*
 * 这是Solution中的总结，挺好的。对于处理一些问题可以有基础的思路，然后在基础思路上进行修改。原版 => 改版
 ...
 * use Breadth-first search for unweighted graphs and use Dijkstra's algorithm for weighted graphs.
 ...
 * The point here is that the properties of the graph and the goal define the kind of algorithms we might be able to use.
 */
...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        // 超时
        typedef tuple<int, int, int> group; // 缩短定义长度
        ...
        priority_queue<group, vector<group>, greater<>> q;
        ...
        while(!q.empty()) {
            ...
            for (int i = 0; i < n; i++) {
                // 这种不做任何比较，直接往里塞然后靠 priority_queue去一个一个处理的太慢了
                // 相当于就是把所有路径全部走了一遍
                if (graph[cur_node][i] > 0) {
                    q.emplace(graph[cur_node][i] + cost, stop + 1, i);
                }
            }
        }
        return -1;
    ...
    /* Dijsktra算法改版
       原版是每次从堆中拿出当前最小的边对应的点，更新所有的邻接点，再把对应的点放到堆中作为下一次使用
       这个题目多了一个k中专点，也就是走k+1步，所以不能每次都找到最佳的最短路径(因为可能最佳的最短路径超过k+1步)
       那么题目处理过程跟原版算法的区别就在于，我们在处理邻接点的时候，不能因为无法更新到该点长度(原版算法)就忽略这个点
       还需要看一下当前到的这个点所用的步数是否小于当前最佳路径的步数， 如果小于的话，就代表从这个点出发到最终终点也是有意义的(因为有可能k+1步内)
       到达终点，我们也需要把这个点对应的路径长度信息放到堆中。这里相对于原版就是，多了一些额外的非最佳路径上的点，主要是用来处理无法在k+1步内实现最佳
       路径的时候的次要路径。
       但是与原版相似的是，在这种贪心下，不管当前处理的点是不是最佳路径上的点，都需要看能否更新到邻接点的距离。

       流程上，
       原版：
       1、从当前堆中，拿出当前距离起点集合最近的点
       2、检查该点是否在起点集合中出现过，出现过则回到1。(因为可以有多条路径到这个点，可能在其他路径上这个点已经最小处理过了，所以这里就跳过)
       3、标记该点为visited。因为根据贪心子问题最优，当堆中选到这个点，那么到这个点就是已经最短的了，用这个点去更新后面的信息。(然而我忘记咋证明了。。)
       4、对于该点的所有邻接点，更新距离，更新逻辑：
            a. 如果[起点集合]到[邻接点]距离 > [起点集合]到[该点] + [该点]到[邻接点]。那么根据贪心子问题最优，就可以更新[起点集合]到[邻接点]的距离了。
            b. 满足a的情况下，将该邻接点放入到堆中，因为这个点对应的距离信息{有可能}成为在最短路径中的一部分，需要在堆中排序。
            p.s. 根据3知道，只有在3那边的点，才是最优点， 经过a.b.处理后的点只是有可能而已，所以要做终点判断应该在3那边做。
       5、更新完回到1继续
       6、当堆用完后，就得到了从起点到所有其他点的最短路径长了。当然如果想要对应某个终点的最短路径长，可以在1-5的循环中，提前判断是否处理到终点了。

       本题改版:
       1、从当前堆中，拿出当前距离起点集合最近、且用步数最短的点 => 即距离相同选步数短的处理(这个其实无所谓。。。距离近是第一位就行)
       2、因为有终点的需求，所以检查该点是否是终点，是终点意味着我们在k+1步内找到了这个点，那么这个情况就是跟原版一样了
       3、检查走到该点是否已经是k+1步了。如果是的话，就跳过不处理，因为不能再走了。因为有这个条件的限制，所有在堆中的点，一定是<= k + 1步的，无法走超过
       4、对于该点的邻接点，更新逻辑：
            a. 如果[起点集合]到[邻接点]距离 > [起点集合]到[该点] + [该点]到[邻接点]。那么这个情况就是原版的情况
               ·更新[起点集合]到[邻接点]的距离。
               ·更新[起点集合]到[邻接点]用的步数。这里记录步数的原因在b中解释。
            b. 反之，我们找到了一个不在最佳路径上的点。这个时候需要看这个不在最佳路径上的点所消耗的步数是否大于最佳路径。
               ·如果一个不在最佳路径上的点，用的步数还比最佳路径用的多，那么就没有继续从这个点往后处理的必要了，可以直接剪枝剪掉
               ·反之如果不在最佳路径上，但用的步数比最佳路径少，那么这个点就是有意义的，因为有可能在k+1步内无法沿着最佳路径走到终点，那么就有可能需要
                从这些有意义的点往下继续走。
            其实这里如果b中不做任何处理，所有邻接点的下一步情况都塞进去，就相当于是把所有情况都处理了，就很慢。。。还是要剪枝的。
        5、6、就跟原版一样了。
        简单来说，改版的意义就在于，把不在最佳路径上的、且有意义的点保留下来，只是为了处理k+1步无法到达终点的情况罢了。

        代码中用的是从-1开始。。因为题目中的k是中转次数， k+1才是步数。。所以为了不写 + 1。。。就放了-1开始。用中转个数来定。
    ...
    int findCheapestPrice1(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        ...
        while(!q.empty()) {
            ...
            for (int i = 0; i < n; i++) {
                if (graph[cur_node][i] > 0) {
                    if (dist[i] > graph[cur_node][i] + cost) {
                        dist[i] = graph[cur_node][i] + cost;
                        steps[i] = stop;
                        q.emplace(dist[i], stop + 1, i);
                    }
                    else if (stop < steps[i]) {
                        // steps[i] = stop;
                        q.emplace(graph[cur_node][i] + cost, stop + 1, i);
                    }
                }
    ...
    // 正向，递推DFS + memo 跟递归的DFS就差几十ms的事情。。剪枝上弱于逆向的递归。。。。致命
    /*
     * 正向逻辑：
     * 每走到一个点，看当前到该点的长度是否比之前同样步数到达该点的长度大。
     * (这里之所以要记录一下到该点对应步数的最短长度，是因为，正向往下DFS的时候，大概率会以相同的步数重复经过很多点。如果不做剪枝的话，就相当于
     * 在处理一条冗余的道路。毕竟如果以相同步数到达同一点后的长度更长的话，就没必要往下继续DFS了。。后面肯定都是长的。要提前剪枝。)
     * 如果长度小于的话，就证明这是一个这个步数下，到该点，更近的长度，记录在memo里为了其他到这里的路径来判断。
     * 然后要看该点是不是终点，是的话就可以不用继续做了。
     * 如果不是终点的话，要看步数是否还有剩余，如果没有剩余，就意味着这条路径的DFS最终找不到终点，那么就停止不继续DFS了。
     *
     * 当上述条件都不满足，进来后，就根据到所有邻接点的长度，加上后继续往下走即可。
     *
     *
     * 这边的缺点是，在长度判断剪枝的时候。。如果第一次就运气最好走到了最佳路径，那么就可以把其他的都能剪掉。。。但是如果每次一都是当前最远的路径的话。
     * 相当于是全跑了一遍(从最远，到最近。。。)，没有任何剪枝。。。这也是超时的原因了。。
     *
     * 其实说白了。。这个做法下。。memo并没有起到很好的剪枝效果(因为要从最坏情况考虑)。只是用于记录了，从src到每个点，在某个步数下的最小长度。。
     * 逻辑上就是全跑了一遍。。。。最终分析出来是这样的。。23333
     */
    int findCheapestPrice2(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        typedef tuple<int, int, int> group; // 缩短定义长度
        vector<vector<int>> graph(n, vector<int>(n, 0));
        vector<vector<int>> memo(n, vector<int>(k + 2, INT_MAX));
        for (auto v : flights) {
            graph[v[0]][v[1]] = v[2];
        }
        dfs2(graph, memo, src, 0, 0, dst, k, n);
        int res = INT_MAX;
        for (int i = 1; i <= k + 1; i++) {
            res = min(memo[dst][i], res);
        }
        return res == INT_MAX? -1 : res;
    }

    void dfs2(vector<vector<int>>& graph, vector<vector<int>>& memo, int cur, int dist, int steps, int dst, int k, int n) {
        if (dist > memo[cur][steps]) return;
        memo[cur][steps] = dist;
        if (cur == dst) return;
        if (steps == k + 1) return;
        ...
        for (int i = 0; i < n; i++) {
            if (graph[cur][i] > 0) {
                dfs2(graph, memo, i, dist + graph[cur][i], steps + 1, dst, k, n);
            }
        }
    }
    ...
    /*
     * 这里顺带记录一下自己的理解，为啥递推的正剪枝---比递归的逆剪枝慢。。。
     * 递推是，我们走到一个点，看当前长度和之前同一个步数相比来决定是否继续往下走，这么一看就感觉，只是大概剪枝了一下，最坏情况就是永远决定往下走，就前面说的每次都是当前最大处理。
     * 因为这个地方是 走到一个点，那么很明显，走到一个点就判断，我可以有很多条路往这个点走进来，那么当然就要多次判断了，并没有达到最优子问题，只是无脑往下找而已。结论上从不带步数k限制
     * 变成了一个带步数k限制的vector而已。这么想就明白了。虽然剪枝了，，但是相对而言少点(可能吧，反正不会证明)。
     *
     * 递归那个相当于是，从最终点出发，倒回来，那么对于倒回来的这个点，他就知道有多少方法能倒回来。。。(不对啊。。这么一想，这和正的一样啊。。)
     * TM就离谱。。分析半天两个感觉上是一样的。。。。。。
     * 不过无论如何。。这题也算是一个复习了。。好多memo的DFS都是递归回来的。。递推感觉还是弱爆了？？？不清楚。。2333
     */

    /*
     * 递归的DFS + memo。 这个相比于递推的那个，就是真实的memo了。做到了很好的剪枝效果，memo做到了DP的效果。这里粘贴一下solution中对一个题能否用dp思想做的一个判断描述，还是挺有用的。
     *
     ...
     *      A base case.
     *
     * 这里memo[i][j] 代表了走到i，用了j步 的时候，最小的路径长度(或者消费也行，毕竟那么多人用cost)不论是否走到了终点。
     *
     * dp 递归逻辑是
     * 1、想要找到从src 用j步 走到 i的最短路径长度
     * 2、子事件即为，用 1 步走到 src的邻接点 adja0， 然后找从adja0 用j - 1步走到i的最短路径长度。
     * 3、遍历所有的邻接点，拿到最小的那个，就是从这个src点 用j步 走到 i的最短路径长度。
     *
     * 剪枝逻辑、终点条件有
     * 因为有步数用完了的限制，只要能进到这个dfs中，就是肯定能正常走到，所以先判断是否终点！！！！
     * a. 当前走到了终点，那么就不用往下走了，因为终点走到终点距离是0。
     * b. 步数用完了，那么就知道不能继续走了，也就是没走到终点，那么返回就是INT_MAX意思就是。。这条路径走到这个步数下，走不到了，就返回了。
     * c. 当前点在这个步数下已经走过一次了，那么根据我们的递归逻辑，当一个点在某一步已经处理过后，就代表从这个点往下走剩下几步(要么走完，要么走到终点)的最短长度已经找到了。
     *    就不需要继续走下去了，以后都是一样的，这里就是memo了，剪枝就在这里。
     *
     *
     * 这种step + 1的逻辑是，从src开始走， 走到 i 用了 j步，但是因为是递归，所以先算得是，从i走完剩下(k - j)步或者走到终点的路径长度。
     * 这个逻辑下memo[i][j] 是代表 从src开始 走j 步到i后，后续最短可以走多短。
     * 比如src = 2, k = 3, memo[1][2] = 10 代表 从2 出发 ，走2步到1后，后续最短可以走10。 注意 递归 递归！！！
     *
     *
     */
    ...
    int findCheapestPrice3(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        vector<vector<int>> graph(n, vector<int>(n, 0));
        vector<vector<int>> memo(n, vector<int>(k + 2)); // 因为数组只能从0开始不能从-1开始，所以最多可以走到k + 1步，那么需要k + 2的长度。
        for (auto v : flights) {
            graph[v[0]][v[1]] = v[2];
        }
        // int res = dfs(graph, memo, src, k + 1, dst, n);
        int res = dfs3(graph, memo, src, 0, k, dst, n);
        return res == INT_MAX? -1 : res;
    }

    int dfs3(vector<vector<int>>& graph, vector<vector<int>>& memo, int cur, int steps, int k, int dst, int n) {
        if (cur == dst) return 0;
        if (steps == k + 1) return INT_MAX;
        if (memo[cur][steps]) return memo[cur][steps];


        int cur_res = INT_MAX;
        for (int i = 0; i < n; i++) {
            if (graph[cur][i] > 0) {
                int child_res = dfs3(graph, memo, i, steps + 1, k, dst, n);
                if (child_res != INT_MAX) {
                    cur_res = min(cur_res, child_res + graph[cur][i]);
                }
            }
        }
        memo[cur][steps] = cur_res;
        return cur_res;
    }

    // 这个跟上面不同的是memo的意义加剪枝的逻辑
    // memo代表，从src出发，走到i后还剩j步，往终点最短可以走多少。
    // 这个逻辑上比上一个更加符合递归的要求。
    // src = 2, k = 3, memo[1][2] = 10 代表从2出发走到了1，还剩2步，最短能走10。 跟上面的意义是不一样的，这个更符合从终点往起点递归退回去的直观感受。

    // 那么这里的剪枝条件就是
    // 1、是否终点
    // 2、是否还有剩余步数 >> 就这里有点意义上的区别而已。
    // 3、是否已经递归走过了
    ...
    int findCheapestPrice4(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        ...
        // int res = dfs(graph, memo, src, k + 1, dst, n);
        int res = dfs4(graph, memo, src, k + 1, dst, n);
        return res == INT_MAX? -1 : res;
    }

    int dfs4(vector<vector<int>>& graph, vector<vector<int>>& memo, int cur, int steps, int dst, int n) {
        if (cur == dst) return 0;
    ...
    // 这个是Bellman-Ford算法的步骤减少版
    // 首先要理解原版的流程
    /*
     * 原版流程：
     * v个顶点，e条边
     * 1、总共执行 v - 1轮
     * 2、每一轮都使用所有的e条边去relax(大概就翻译成放缩吧)src与各个点的当前最短距离。
     *
     * 这个执行上虽然看不出来，但是实际上这就是一个bottom-top的dp算法。可以看这边的解释
     * https://www.geeksforgeeks.org/bellman-ford-algorithm-dp-23/
     *
     * 可以这么看。假设有这么一种图,不好画带箭头的，就默认箭头都是从 序列小 指向序列大的就行了。
     *     B -- C
     *   / |    |
     * A - F -- G
     *   \ |    |
     ...
     *
     *
     * 我们来看流程中的第二步，用所有的边去进行relax。
     * 如果：
     * a. 先选了 AB， AD，AF 再选了BF DF， 那我们就成功的在一次循环里更新了两次A到F的距离。
     * b. 先选了 BF DF，再选了 AB AF AD。 那我们就只能更新一次，因为 BF DF开始不与 A接壤，无法更新。
     * 那么这里就是核心了。如果每次都是最差情况呢？？是不是就相当于：
     * 第一轮循环只能用一条边去更新 也就是 AB AF AD，
     * 第二轮循环类似，只能用两条边，多的部分是 BF DF BC FG DE
     * 第三轮是 CG GE
     *
     * 然后每次更新的时候，都要跟上一次的结果做对比，来看是否能变小，变小则存储。。这不就成了 dp了么。
     * 1、这是一个recursive的也就是递推从1条到2条到3条
     * 2、多个选择。对于2条边的第二轮，如果能够更新，则更新，反之不更新。取决于多的这个第二条边带来的影响
     * 3、最优子事件。 如果第一轮后有当前最短，那么第二轮更新完后也是当前最短，最终就能更到最终轮的当前最短。
     * 4、base case，基础事件。 也就是所谓的 第0轮，自己到自己是0就是开始了。然后一轮一轮更新下去。
     * (虽然写这么多都是屁话。。。每次看到dp也不知道是dp去做。。但是怎么样呢。。就是写下来呗。万一哪天开窍了呢)
     *
     *
     *
     * 好 那么看对于这个题我们要把原版代码做什么修改
     * 原版代码使用一个vector直接就更新就行了，因为他没有k+1步的条件限制，也就是k+1条边的条件限制。所以无论上面的 ab情况先后哪个都不影响原版算法。
     * 修改点就是：
     * 我们需要强制让这个vector只能一轮一轮改变，而不能在同一轮做多条边的改变。
     * 比如上面的图 假设 AB = 1, AF = 7, AD = 3, BF = 2, DF = 2
     * 我们只能让AB AF AD发挥作用，而不能让BF DF发挥作用。
     * 如何做到这点？
     * 从原版代码我们知道，原版是直接更新了最初的那个vector
     * 也就是说，当我们更新了AB后，B就有值了，在碰到BF就可以继续更新了。但其实BF不应该看到这个B的值。
     * 所以这里我们就明白了，不能在一个vector里面修改，需要留一个vector来保留上轮的结果。
     * 然后把新结果全部写到新的vector中作为本轮结果。
     * 旧的vector提供什么？
     * dist[u]的值 -> 说白了就是之前有没有更新到u这个点。因为我们的目标是一轮一轮更新，所以如果前面更新到了，那么后面就有可能会被用到
     * 就这个值就行了。剩下的全部在新的vector中更新即可。
     * 同一轮更新同一个点多次有问题吗？
     * 没有。。因为在这种新旧vector的控制下。。只有可能是与上个vector中的顶点接壤的(也就是有非INT_MAX值的)才会参互relax。
     * 所以不管更新多少次，也是同一轮内消耗同样的步数(或者说边数)来做的事情，符合要求。
     *
     *
     * 其实核心还是要理解Bellman-Ford的核心原理。。。以前还是太肤浅了。。年轻如我。。
     */

    int findCheapestPrice5(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        vector<int> dist(n, INT_MAX);
        dist[src] = 0;
        ...
        for (int i = 0; i <= k; i++) {
            ...
            for (auto edge : flights) {
                ...
                // 就这里 dist[u] != INT_MAX 还有cur[v]做修改，保证了同轮中不会有多余的步数被消耗！！！
                if (dist[u] != INT_MAX && cur[v] > dist[u] + weight) {
                    cur[v] = dist[u] + weight;
                }
            }
            dist = cur;
    ...
    /*
     * 算是复习一下怎么BFS遍历图
     * 1、找到起始点放进queue里，记录visited
     * 2、pop一个，处理所有的邻接点，记录visited，防止循环找
     * 3、做到queue为空为止
     *
     * 如果要一层一层的处理，那就不能直接pop一个处理一个。需要控制一下次数。
     * 比如第一层queue 1个点 那就是1此循环
     * 然后3个邻接点，那么第二层 queue就是3个点， 之后就3次循环。
     * 依次类推。这样子是按层序遍历(拓扑、树都挺好用的，毕竟有向无环图不是。。)，一层一层的。 这种不需要visited。 因为按层可以转来转去，没关系。。。大概率这种题都有提前结束的条件提供。
     * 上面那个是基础广度，就能拆先拆开那种。
     *
     * 那么到这个题，因为限制了步数，很明显要用层序，得一层一层来，也就是一步一步来。需要看每一步走到哪里之类的。
     * 。。。。。。删了半天，好像层序遍历也可以一直遍历下去的，因为如果图不带那种negative的环，按照限制条件，最终queue里面就没点了。
     *
     * 说回这个题
     * 一层就是一步，如果不限制步数，那么就正常逻辑更新：
     * 遇到一个邻接点 -> 能够缩小到src 到 该邻接点的距离(也就是用这条边能不能更新) -> 更新且加入queue 因为未来可能用到更新别的路径。
     *
     * 这里限制步数了，就不能单纯的跟最优的去比(上面的逻辑是永远维持一个最优的也就是最短的路径长度)，因为最优的在限制步数内不一定能到，那就得从非最优的继续走起来。
     * 所以这里过程中，用dist[][]来控制。
     * 用二维的原因就是，一维的永远只保证最优的非最优就没了。 加上了二维，就可以把最优、非最优等所有情况。也就是走k步的所有情况都可以放进去。
     * 说白了就是往下走k步的bfs全遍历。然后更新的时候，不单纯更新最优，只更新该步数下最优即可。这样子非最优的也会因为第一次的相遇，比初始化的INT_MAX小而加入queue。
     * 然后加入未来的判断中。
     *
     * 这样子更新到最后。我们就知道dist[dst][0....k+1]
     * 也就是我们得到了所有步数下src到dst的最优距离(注意前提是所有可能的步数下)。然后选择里面最小的那个就行啦。。
     *
     * 核心其实就是。。普通的层序遍历(层序也是bfs！！诶嘿) 再加 了个k步所有情况的额外存储就是了。。。。
     *
     */

    int findCheapestPrice6(int n, vector<vector<int>>& flights, int src, int dst, int k) {
